...
function calcHand(hand) {
    ...
    return hand.filter(Boolean).reduce((acc, c) => {
    ...
    g.playerOrder.forEach(id => {
        ...
        g.playerOrder.forEach(pid => {
            ...
            view.players[pid] = {
                ...
                handCount: g.players[pid].hand.filter(Boolean).length,
                hand: pid === id
                    ? g.players[pid].hand
                    : null
            };
...
function nextTurn(g) {
    ...
    g.turnIndex =
        (g.turnIndex + 1) % g.playerOrder.length;
...
function resolveRound(g, callerId) {
    ...
    const kamoFail =
        g.playerOrder.some(id =>
            id !== callerId &&
            scores[id] <= callerScore
        );
    ...
    g.playerOrder.forEach(id => {
        ...
        if (kamoFail && id === callerId) {
            ...
            kamoFail &&
            id !== callerId &&
            scores[id] === Math.min(...Object.values(scores))
        ) {
        ...
        g.totals[id] =
            (g.totals[id] || 0) + ptsThisRound;

        if (g.totals[id] >= g.rules.limit) {
            g.pendingLosers.push(id);
        }
        ...
        results[id] = {
            ...
            total: g.totals[id],
            badge,
            hand: g.players[id].hand.filter(Boolean)
        };
    });
...
io.on("connection", socket => {
    ...
    socket.on("joinGame", ({ name, room }) => {
        ...
        if (!roomCode) {
            ...
            rooms[roomCode] =
                createGameState(socket.id, roomCode);
        }
        ...
        if (!g.playerOrder.includes(socket.id)) {
    ...
    socket.on("updateRules", rulesData => {
        ...
        if (
            ...
            g.rules = {
                limit: parseInt(rulesData.limit),
                straf: {
                    n: parseInt(rulesData.strafN),
                    t: rulesData.strafT
                }
            };
    ...
    socket.on("discard", ({ indices }) => {
        ...
        g.lastActionText =
            `${g.players[socket.id].name} gooide ${cards.map(c => cardLabel(c.v) + c.s).join(", ")}`;
    ...
    socket.on("drawFromDeck", () => {
        ...
        // extra set kaart automatisch neerleggen
        if (
            g.lastDiscard?.type === "set" &&
            g.lastDiscard.player === socket.id &&
            card.v === g.lastDiscard.cards[0].v
        ) {
            ...
            g.lastActionText =
                `${g.players[socket.id].name} trok een ${cardLabel(card.v)} en gooide deze direct bij!`;
        ...
        g.lastActionText =
            `${g.players[socket.id].name} pakte een blinde kaart.`;
    ...
    // ─────────────────────────
    // BELANGRIJKE FIX:
    // bovenste open kaart zichtbaar houden
    // ─────────────────────────
    ...
    socket.on("drawFromOpen", () => {
        ...
        // pak ALTIJD de bovenste kaart
        if (g.tableStack.length === 0) {
        ...
        g.lastActionText =
            `${g.players[socket.id].name} pakte ${cardLabel(card.v)}${card.s || ""} van de open stapel.`;
    ...
    socket.on("invallen", ({ index }) => {
        ...
        if (
            ...
            g.lastActionText =
                `${g.players[socket.id].name} viel in met ${cardLabel(c.v)}${c.s}!`;
    ...
    socket.on("call", () => {
        ...
        if (
            g &&
            socket.id === g.turn &&
            calcHand(g.players[socket.id].hand) <= 5
        ) {

            resolveRound(g, socket.id);
        }
    });
