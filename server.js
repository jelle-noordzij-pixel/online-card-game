...
io.on("connection", (socket) => {
    socket.on("joinGame", (name) => {
        gameState.players[socket.id] = { id: socket.id, name: name || "Speler", hand: [], score: 0 };
        if (!gameState.playerOrder.includes(socket.id)) gameState.playerOrder.push(socket.id);
        io.emit("updateState", gameState);
    });

    // We noemen de actie nu simpelweg 'GO' om verwarring te voorkomen
    socket.on("GO", () => {
        console.log("START SIGNAAL ONTVANGEN!"); 
        gameState.status = 'PLAYING';
        gameState.deck = buildDeck();
        gameState.tableStack = [gameState.deck.pop()];
        
        gameState.playerOrder.forEach(id => {
            if (gameState.players[id]) gameState.players[id].hand = gameState.deck.splice(0, 5);
        });
...
server.listen(PORT, () => console.log("Server is gestart"));
